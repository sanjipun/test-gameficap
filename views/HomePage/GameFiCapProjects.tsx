...
const GameFiCapProjects = () => {
  ...
  return (
    <section id="game-fi-cap-projects" className="bg-C4C4C4 bg-opacity-15 text-primary leading-6">
      <div className="max-w-1440 mx-auto px-5 sm:px-7 md:px-10 lg:px-20 py-14">
        ...
        <div className="py-9 flex justify-start flex-wrap items-center">
          {TAGS.map((tag) => (
            <h1
              onClick={() => setCurrentTag(tag.name)}
              className={`mr-5 ${
                tag.name === currentTag ? "bg-E3E9EE" : "bg-FAFAFA"
              } px-8 py-3 rounded cursor-pointer m-2 ml-0`}
              key={tag.name}
            >
              {tag.name}
            </h1>
